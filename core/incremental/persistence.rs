...
/// returns `Done`: `IOResult::IO` means "call me again", so advancing first
/// abandons an in-flight balance. The `sought` flag lets re-entry re-drive the
/// mutating op without re-running the seek.
#[derive(Debug, Default)]
pub enum WriteRow {
    #[default]
    GetRecord,
...
mod tests {
    ...
    use crate::{Connection, Database, Pager, SqliteDialect, IO};
    use std::sync::Arc;
    ...
    fn create_test_pager() -> (Arc<Pager>, i64, i64) {
        ...
        let db =
            Database::open_file(io.clone(), ":memory:", Arc::new(crate::SqliteDialect)).unwrap();
    ...
    /// Resuming `WriteRow` at `InsertIndex` must place the new row's index entry
    /// correctly even when the index cursor was left elsewhere: `sought: false`
    /// makes the arm re-seek before inserting.
    #[test]
