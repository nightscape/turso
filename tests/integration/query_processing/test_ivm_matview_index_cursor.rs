...
fn assert_refused(
    ...
    let err = conn
        .execute(sql)
        .expect_err(&format!("{what}: expected a refusal, the query succeeded: {sql}"))
        .to_string();
    for needle in [
        index,
        view,
...
fn indexed_read_is_refused_only_while_that_view_has_uncommitted_changes(
    ...
    conn.execute("BEGIN")?;
    assert_eq!(limbo_exec_rows(&conn, point_read), baseline, "open tx, no writes");
    conn.execute("COMMIT")?;

    // (2) Writes to an unrelated TABLE must not refuse.
    conn.execute("BEGIN")?;
    conn.execute("INSERT INTO unrelated (x) VALUES (1)")?;
    assert_eq!(limbo_exec_rows(&conn, point_read), baseline, "unrelated table write");
    conn.execute("COMMIT")?;
    ...
    assert_refused(&conn, "after writing this view's base", point_read, "v", "idx_v_id");
    ...
    assert_eq!(limbo_exec_rows(&conn, point_read), baseline, "after ROLLBACK");
    assert!(
...
fn writes_to_an_indexed_matview_still_work_inside_a_transaction(
    ...
    conn.execute("BEGIN")?;
    for i in 10..20 {
        conn.execute(&format!("INSERT INTO t_raw (id, st) VALUES ('c{i}', 'TODO')"))?;
    }
    ...
    assert_eq!(
        limbo_exec_rows(&conn, "SELECT count(*) FROM v WHERE id >= 'c10' AND id < 'c99'"),
...
#[turso_macros::test(views)]
fn chained_matview_over_an_indexed_view_is_unaffected(
    tmp_db: TempDatabase,
) -> anyhow::Result<()> {
    let conn = tmp_db.connect_limbo();
    setup(&conn)?;
...
fn chained_view_with_its_own_index_is_refused(tmp_db: TempDatabase) -> anyhow::Result<()> {
    ...
    for (id, st) in ROWS {
        conn.execute(&format!("INSERT INTO t_raw (id, st) VALUES ('{id}', '{st}')"))?;
    }
...
fn both_levels_indexed_are_both_refused(tmp_db: TempDatabase) -> anyhow::Result<()> {
    ...
    conn.execute("BEGIN")?;
    conn.execute("INSERT INTO t_raw (id, st) VALUES ('b9', 'NEW')")?;
    assert_refused(&conn, "both indexed, reading v", "SELECT count(*) FROM v", "v", "idx_v_id");
    Ok(())
}
...
fn c1_target_uncommitted_rows_visible_through_the_index(
    ...
    assert_arms_agree(&conn, "C1: count inside a write transaction", "SELECT count(*) FROM @B@");
    assert_arms_agree(
        &conn,
