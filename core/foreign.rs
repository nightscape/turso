...
pub trait ForeignDataWrapper: std::fmt::Debug + Send + Sync {
    ...
    ///
    /// The engine re-parses this string to derive the table's columns, so every
    /// identifier must be quoted (`ast::Name::exact(name).as_ident()`). Raw
    /// interpolation turns a reserved-word name into a syntax error and a
    /// spaced name into a *different* column, because a declared type may
    /// itself be multi-word (`UNSIGNED BIG INT`).
