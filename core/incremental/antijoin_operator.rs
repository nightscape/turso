...
//! discipline matters. Each state issues exactly one btree operation per
//! poll and advances only after it returns `Done`, so a yielded op is
...
use crate::types::{IOResult, IOResultOr, ImmutableRecord, SeekKey, SeekOp, SeekResult};
        ...
        Some(rec) => match rec.get_three_values(0, 1, 2) {
            Ok((v0, v1, _v2)) => {
                ...
                let hash_ok = match v1.to_owned()? {
                    Value::Blob(ref b) => Hash128::from_blob(b)
                        .map(|h| h == join_key_hash)
                        .unwrap_or(false),
                    _ => false,
                };
                sid_ok && hash_ok
                    ...
                    RKeyScan::Idle => {
                        *outer =
                            EvalState::Antijoin(Box::new(AntijoinEvalState::EmitRTransitions {
                                ...
                                affected_keys,
                                key_idx,
                                sub: Box::new(RKeyScan::ReadingRCount),
                            }));
