...
pub struct Schema {
    ...
    /// `CREATE INDEX` rows whose table is a materialized view, held by
    /// `populate_indices` until `populate_materialized_views` registers the view.
    pub(crate) deferred_matview_indexes: Vec<UnparsedFromSqlIndex>,
...
impl Schema {
    ...
    /// The secondary indexes on a materialized view, as the view's delta
    /// applier maintains them.
    pub fn matview_indexes(
        &self,
        view_name: &str,
    ) -> Vec<crate::incremental::compiler::MatviewIndex> {
        self.get_indices(view_name)
            .map(|idx| {
                turso_assert!(
                    idx.columns.iter().all(|c| c.expr.is_none()),
                    "expression index on a materialized view is refused at CREATE INDEX",
                    { "index": idx.name, "view": view_name }
                );
                turso_assert!(
                    idx.where_clause.is_none(),
                    "partial index on a materialized view is refused at CREATE INDEX",
                    { "index": idx.name, "view": view_name }
                );
                crate::incremental::compiler::MatviewIndex {
                    index: idx.clone(),
    ...
    pub fn populate_indices(
        ...
        for unparsed_sql_from_index in from_sql_indexes {
            ...
            let Some(table) = self.get_btree_table(table_name) else {
                if known_matview_names.contains(&self.normalize_table_lookup_name(table_name)) {
    ...
    fn attach_deferred_matview_indexes(&mut self, syms: &SymbolTable) -> Result<()> {
        for unparsed in std::mem::take(&mut self.deferred_matview_indexes) {
            let Some(table) = self.get_btree_table(&unparsed.table_name) else {
                // The view is already reported unusable, and so is its index.
                if self.is_unusable_view(&self.normalize_table_lookup_name(&unparsed.table_name)) {
                    continue;
                }
                return Err(LimboError::Corrupt(format!(
                    "sqlite_schema contains index for materialized view '{}', which neither \
                     loaded nor was reported unusable: rootpage={} sql={}",
                    unparsed.table_name, unparsed.root_page, unparsed.sql,
                )));
            };
...
impl TryClone for Schema {
    ...
    fn try_clone(&self) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            incompatible_views,
            deferred_matview_indexes: self.deferred_matview_indexes.try_clone()?,
