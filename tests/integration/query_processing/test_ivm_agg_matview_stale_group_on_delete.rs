...
async fn assert_single_row(conn: &turso::Connection, label: &str) -> anyhow::Result<()> {
    ...
    assert_eq!(n, 1, "[{label}] matview must have exactly 1 row for 'x', got {n}");
    Ok(())
}
...
async fn test_ivm_agg_matview_stale_group_on_delete() -> anyhow::Result<()> {
    ...
    conn.execute("DELETE FROM j1 WHERE base_id = 'x'", ()).await?;
...
async fn test_ivm_agg_three_chain_tags_middle_toggle() -> anyhow::Result<()> {
    ...
    .await?;
    conn.execute("CREATE TABLE reqs (block_id TEXT, required_id TEXT, PRIMARY KEY (block_id, required_id))", ()).await?;
    conn.execute("CREATE TABLE tags (block_id TEXT, tag TEXT, PRIMARY KEY (block_id, tag))", ()).await?;
    conn.execute("CREATE TABLE adv (anchor_id TEXT, lesson_id TEXT, PRIMARY KEY (anchor_id, lesson_id))", ()).await?;
    ...
    conn.execute("INSERT INTO tags (block_id, tag) VALUES ('x', 'Page')", ()).await?;
...
async fn test_ivm_agg_multiblock_interleaved_toggle() -> anyhow::Result<()> {
    ...
    let db = Builder::new_local(path)
        ...
        .await?;
    let conn = db.connect()?;
    conn.execute("CREATE TABLE base (id TEXT PRIMARY KEY, content TEXT, updated_at INTEGER)", ()).await?;
    conn.execute("CREATE TABLE reqs (block_id TEXT, required_id TEXT, PRIMARY KEY (block_id, required_id))", ()).await?;
    conn.execute("CREATE TABLE tags (block_id TEXT, tag TEXT, PRIMARY KEY (block_id, tag))", ()).await?;
    conn.execute("CREATE TABLE adv (anchor_id TEXT, lesson_id TEXT, PRIMARY KEY (anchor_id, lesson_id))", ()).await?;
    conn.execute("CREATE MATERIALIZED VIEW reqs_agg AS SELECT block_id AS source_id, json_group_array(required_id) AS vals FROM reqs GROUP BY block_id", ()).await?;
    ...
    for b in ["P", "C", "R", "A"] {
        conn.execute(&format!("INSERT INTO base VALUES ('{b}', 'c0', 0)"), ()).await?;
    }
    conn.execute("INSERT INTO reqs (block_id, required_id) VALUES ('P', 'R')", ()).await?;
    conn.execute("INSERT INTO tags (block_id, tag) VALUES ('P', 'ctx'), ('P', 'task')", ()).await?;
    conn.execute("INSERT INTO adv (anchor_id, lesson_id) VALUES ('P', 'A')", ()).await?;
    conn.execute("INSERT INTO reqs (block_id, required_id) VALUES ('C', 'P')", ()).await?;

    async fn no_dup(conn: &turso::Connection, label: &str) -> anyhow::Result<()> {
        let mut rows = conn.query("SELECT id, COUNT(*) c FROM m GROUP BY id HAVING COUNT(*)>1", ()).await?;
    ...
    for i in 1..=6 {
        conn.execute(&format!("UPDATE base SET content='c{i}', updated_at={i} WHERE id='P'"), ()).await?;
        conn.execute("DELETE FROM tags WHERE block_id='P'", ()).await?;
        no_dup(&conn, &format!("r{i} tags-del")).await?;
        conn.execute("INSERT INTO tags (block_id, tag) VALUES ('P','ctx'),('P','task')", ()).await?;
        no_dup(&conn, &format!("r{i} tags-ins")).await?;
        conn.execute("DELETE FROM reqs WHERE block_id='P'", ()).await?;
        no_dup(&conn, &format!("r{i} reqs-del")).await?;
        conn.execute("INSERT INTO reqs (block_id, required_id) VALUES ('P','R')", ()).await?;
        no_dup(&conn, &format!("r{i} reqs-ins")).await?;
        conn.execute("DELETE FROM adv WHERE anchor_id='P'", ()).await?;
