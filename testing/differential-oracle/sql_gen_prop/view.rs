...
enum ViewSelectKind {
    ...
    ///   SELECT id, parent, 0 FROM base
    ///   UNION ALL
    ///   SELECT base.id, base.parent, tree.depth+1
    ///   FROM base JOIN tree ON base.parent = tree.id
    ///   LEFT OUTER JOIN boundary bt ON bt.bcol = tree.id
    ///   WHERE tree.depth < N AND (tree.depth = 0 OR bt.bcol IS NULL)
    /// ) SELECT * FROM tree
    ///
    /// A recursive-descendants view whose recursive arm contains a LEFT OUTER
    /// JOIN *anti-join* against a boundary table (`bt.bcol IS NULL`). This
    /// mirrors holon's `watch_real`/`focus_descendants` matview: the LEFT JOIN
    /// compiles the shared recursive-step join into a DAG *diamond* (matched
    /// half + null-extended antijoin half), the only shape that exercises the
    /// per-pass `exec_node_cache` dedupe of a doubly-referenced operator inside
    /// the recursive fixed point. With enough base rows to push the operator's
    /// persistent state btree past a leaf page, an I/O yield can land between the
    /// two consumers' descents and the shared node is committed twice, leaving a
    /// residual stale edge that surfaces as a phantom/duplicate recursive row
    /// (holon recursive-matview corruption, 2026-07). No prior generated view
    /// put an anti-join *inside* the recursive arm, so this bug class was unreachable.
    RecursiveAntiJoinDescendants,
...
fn create_view_inner(schema: &Schema, materialized: bool) -> BoxedStrategy<CreateViewStatement> {
    ...
    let has_three_sources = all_sources.len() >= 3;
    let has_int_pair = all_sources.iter().any(|t| has_integer_pair(t));

    let complex_filter_joins: Vec<ComplexFilterJoinShape> = all_sources
        .iter()
        .flat_map(|left| {
            all_sources
                .iter()
                .filter_map(move |right| complex_filter_join_shape(left, right))
        })
        .collect();
    let has_complex_filter_join = !complex_filter_joins.is_empty();
    let complex_filter_joins_rc = std::rc::Rc::new(complex_filter_joins);
    ...
    (
        ...
        proptest::sample::select(all_sources),
        {
            ...
            // The anti-join-in-recursive-arm shape needs an int pair (id/parent)
            // for the tree plus a second table to serve as the anti-join
            // boundary. Push it a few times: it is the only path to the
            // recursive-diamond bug class and is otherwise a rare draw.
            if has_int_pair && has_multiple_sources {
                kinds.push(Just(ViewSelectKind::RecursiveAntiJoinDescendants));
                kinds.push(Just(ViewSelectKind::RecursiveAntiJoinDescendants));
                kinds.push(Just(ViewSelectKind::RecursiveAntiJoinDescendants));
            }
        ...
        .prop_flat_map(move |(if_not_exists, view_name, table, kind)| {
            ...
            let complex_filter_joins_inner = complex_filter_joins_rc.clone();
            ...
            match kind {
                ...
                ViewSelectKind::ComplexFilterJoin => {
                    let shapes = (*complex_filter_joins_inner).clone();
                    (proptest::sample::select(shapes), 0u32..4u32)
                        .prop_map(move |(shape, pred_kind)| {
                            let t1_name = shape.left.qualified_name();
                            let t2_name = shape.right.qualified_name();
                            let t1_col = &shape.left_key;
                            let t2_col = &shape.right_key;
                            let shared_col = &shape.right_shared.name;
                            ...
                            CreateViewStatement {
                                ...
                                output_columns: vec![
                                    ColumnDef::new("cfj_l", shape.left_shared.data_type),
                                    ColumnDef::new("cfj_r", shape.right_shared.data_type),
                                ],
                            }
                        })
                        .boxed()
                ...
                ViewSelectKind::RecursiveAntiJoinDescendants => {
                    let int_cols: Vec<(String, String)> = {
                        let ints: Vec<&ColumnDef> = table_col_defs
                            .iter()
                            .filter(|c| c.data_type == DataType::Integer)
                    ...
                    let (id_col, parent_col) = int_cols[0].clone();
                    let base_table_name = table_name;
                    let base_name_only = table.name.clone();
                    let all = (*all_sources_inner).clone();
                    (proptest::sample::select(all), 3u32..=10u32)
                        .prop_map(move |(boundary, max_depth)| {
                            // Prefer a boundary table distinct from the base so
                            // this is a genuine anti-join; if the draw lands on
                            // the base itself (or an empty table) fall back to
                            // the base as its own boundary — still a LEFT OUTER
                            // JOIN diamond, never a rejection (keeps the Union
                            // strategy's reject budget clean).
                            let use_boundary = if boundary.name != base_name_only
                            ...
                            // Boundary join column: an integer column so it
                            // compares against the tree's integer id; fall back
                            // to the first column otherwise.
                            let bcol = use_boundary
                                .columns
                                .iter()
                                .find(|c| c.data_type == DataType::Integer)
                                .or_else(|| use_boundary.columns.first())
                                .map(|c| c.name.clone())
                                .unwrap_or_else(|| id_col.clone());
                            let bt = use_boundary.qualified_name();
                            let bt_alias = if bt == base_table_name { "btself" } else { "bt" };
                            // Explicit tree(id, parent_id, depth) header so the
                            // recursive term's projected columns do not collide
                            // by bare name with the base table's.
                            let cte = format!(
                                "WITH RECURSIVE tree(id, parent_id, depth) AS (\
                                 SELECT {id_col}, {parent_col}, 0 AS depth FROM {base_table_name} \
                                 UNION ALL \
                                 SELECT {base_table_name}.{id_col}, {base_table_name}.{parent_col}, tree.depth + 1 \
                                 FROM {base_table_name} JOIN tree ON {base_table_name}.{parent_col} = tree.id \
                                 LEFT OUTER JOIN {bt} {bt_alias} ON {bt_alias}.{bcol} = tree.id \
                                 WHERE tree.depth < {max_depth} \
                                 AND (tree.depth = 0 OR {bt_alias}.{bcol} IS NULL)\
                                 ) "
                            );
                            format!("{cte}SELECT * FROM tree")
                        })
                        .prop_map(move |select_sql| CreateViewStatement {
                            materialized,
                            if_not_exists,
...
#[derive(Debug, Clone)]
struct ComplexFilterJoinShape {
    left: TableRef,
    right: TableRef,
    left_key: String,
    right_key: String,
    left_shared: ColumnDef,
    right_shared: ColumnDef,
}

fn complex_filter_join_shape(left: &TableRef, right: &TableRef) -> Option<ComplexFilterJoinShape> {
    if left.name == right.name {
        return None;
    }
    // Prefer the primary key as the join key: INTEGER PRIMARY KEY aliases the
    // rowid and takes small sequential values, so rows from two independently
    // generated tables actually pair up.
    let join_key = |t: &TableRef| {
        t.columns
            .iter()
    ...
    // Project a column whose bare name exists on both sides but is not the
    // join key: projecting the key would make both sides equal by
    ...
    let right_shared = right
        ...
        .find(|c| {
            c.name != right_key
                && c.name != left_key
                && left.columns.iter().any(|o| o.name == c.name)
        })?
        .clone();
    let left_shared = left
        .columns
        .iter()
        .find(|c| c.name == right_shared.name)
        .expect("right_shared was chosen because the left side has a column of the same name")
        .clone();
    Some(ComplexFilterJoinShape {
        left: left.clone(),
        right: right.clone(),
        left_key,
        right_key,
...
mod tests {
    ...
    #[test]
    fn every_join_shape_is_generated_when_the_schema_supports_it() {
        ...
        let sqls: Vec<String> = (0..2000)
            .map(|_| strategy.new_tree(&mut runner).unwrap().current().select_sql)
            .collect();
        assert!(sqls.iter().any(|sql| sql.contains("AS cfj_l")));
        assert!(sqls.iter().any(|sql| sql.contains("AS bcol")));
    }
}
