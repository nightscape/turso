...
async fn test_view_change_callback_multi_connection() {
    ...
    let builder = Builder::new_local(":memory:").experimental_materialized_views(true);
    let db = builder.build().await.unwrap();
    ...
    let _callback_id = conn_a
        .set_change_callback(move |event: &turso::RelationChangeEvent| {
    ...
    assert_eq!(
        changes.len(),
        1,
        "Callback should fire when connection B commits"
    );
...
async fn test_view_change_callback_multiple_callbacks() {
    ...
    let _id1 = conn_a
        .set_change_callback(move |_event: &turso::RelationChangeEvent| {
    ...
    assert!(
        *callback1_called.lock().unwrap(),
        "Callback 1 should have been called"
    );
    assert!(
        *callback2_called.lock().unwrap(),
        "Callback 2 should have been called"
    );
}
...
async fn test_view_change_callback_removal() {
    ...
    assert_eq!(
        *callback_count.lock().unwrap(),
        1,
        "Callback should fire before removal"
    );

    conn.clear_change_callback(callback_id).unwrap();
    ...
    assert_eq!(
        *callback_count.lock().unwrap(),
        1,
        "Callback should NOT fire after removal"
    );
}
...
async fn test_view_change_callback_clear_all() {
    ...
    conn_a.clear_all_change_callbacks().unwrap();
    ...
    assert_eq!(
        *callback1_count.lock().unwrap(),
        1,
        "Callbacks should NOT fire after clear_all"
    );
    assert_eq!(
...
async fn test_view_change_callback_column_names() {
    ...
    .unwrap();
    conn.execute(
        "CREATE MATERIALIZED VIEW users_view AS SELECT id, name, age FROM users",
        (),
    )
    .await
    .unwrap();
