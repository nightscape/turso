...
pub struct ValueProfile {
    ...
    /// Range integer values are drawn from.
    pub integer_range: RangeInclusive<i64>,
    /// Small domain mixed into text and integer draws so separate statements repeat values.
...
pub struct NarrowValueProfile {
    pub text_pattern: String,
    pub integer_range: RangeInclusive<i64>,
    /// Draw weights of this domain vs. the full `ValueProfile` domain.
...
impl ValueProfile {
    ...
    /// Builder method to draw mostly from a small domain and sometimes from the full one.
...
/// Generate an integer value, mixing in the narrow domain when configured.
pub fn integer_value(value_profile: &ValueProfile) -> BoxedStrategy<SqlValue> {
    let full = value_profile.integer_range.clone();
    match &value_profile.narrow {
        ...
        .boxed(),
        None => full.prop_map(SqlValue::Integer).boxed(),
    }
}
...
/// Generate a text value, mixing in the narrow domain when configured.
pub fn text_value(profile: &StatementProfile) -> BoxedStrategy<SqlValue> {
    ...
    match &value_profile.narrow {
        Some(narrow) => {
            let narrow_strategy = string_regex(&narrow.text_pattern).unwrap();
            prop_oneof![
                narrow.narrow_weight => narrow_strategy,
                narrow.full_weight => full,
            ]
