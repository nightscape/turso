...
// IGNORED: with the MatchCounter I/O re-entry bug fixed, this test now
// exposes a separate, downstream bug in `aggregate_operator.rs:1470` —
// `json_group_array multiset went negative for col 3 val ... — delta
// consolidation invariant violated`. The MatchCounter fix is verified by
// `test_match_counter_no_aggregation` below; un-ignore this once the
// json_group_array delta-consolidation invariant is fixed.
#[tokio::test]
#[ignore = "exposes downstream json_group_array multiset bug post-MatchCounter-fix"]
...
/// Simpler reproducer that targets the MatchCounter I/O re-entry bug in
/// isolation, with no aggregation on top. The original
/// `dual_left_cdc_burst` test exposes a downstream `json_group_array`
/// bug once MatchCounter is fixed; this variant lets us prove MatchCounter
/// alone is correct.
///
/// Same trigger shape (file-backed DB + dual LEFT JOIN + DELETE bursts
/// driving R_COUNT across zero per key), but the matview is just a plain
/// row projection so only the LEFT JOIN operators run.
#[tokio::test]
async fn test_match_counter_no_aggregation() -> anyhow::Result<()> {
    let db_path = "/tmp/turso-match-counter-no-agg.db";
    let _ = std::fs::remove_file(db_path);
    let _ = std::fs::remove_file(format!("{}-wal", db_path));
    let _ = std::fs::remove_file(format!("{}-shm", db_path));
    ...
    // Dual LEFT JOIN, no aggregation. Each LEFT JOIN spawns one
    // MatchCounterOperator; the cartesian shape gives plenty of opportunity
    // for I/O suspension during a single commit.
    conn.execute(
        "CREATE MATERIALIZED VIEW block_matview AS \
         SELECT b.id, b.content, bt.tag, tb.blocker_id \
         FROM block_raw b \
         LEFT OUTER JOIN block_tags    bt ON bt.block_id   = b.id \
         LEFT OUTER JOIN task_blockers tb ON tb.blocked_id = b.id",
        (),
    ...
    // DELETE burst that drives R_COUNT across zero per key — forces
    // MatchCounterOperator into the ScanningL phase repeatedly.
    for i in 0..200 {
        conn.execute(
            &format!("DELETE FROM block_tags WHERE block_id = 'b{i}'"),
            (),
    ...
    // After all deletes, every row in the matview should have NULL tag and
    // NULL blocker_id (the null-pad LEFT JOIN result).
    let mut rows = conn
        .query(
            "SELECT id, tag, blocker_id FROM block_matview WHERE id = 'b0'",
            (),
        )
        .await?;
    let row = rows.next().await?.expect("b0 must still be present");
    let id: String = row.get(0)?;
    assert_eq!(id, "b0");
    // tag and blocker_id must both be NULL after all matching R-side rows
    // were deleted.
    let tag: Option<String> = row.get(1)?;
    let blocker: Option<String> = row.get(2)?;
    assert_eq!(
        tag, None,
        "after deleting all of b0's tags, tag must be NULL"
    );
    assert_eq!(
        blocker, None,
        "after deleting all of b0's blockers, blocker_id must be NULL"
    );
