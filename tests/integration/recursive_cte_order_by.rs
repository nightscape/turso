...
//! panic the access-method / sort-elimination optimizer (see the `Subquery`
//! access-method handling in `core/translate/optimizer/order.rs`).
//!
...
//! table, with the outer query ordering the result.
...
/// Must not panic; every seeded descendant comes back, ordered by (sort_key, id).
#[test]
