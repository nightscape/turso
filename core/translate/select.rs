...
use crate::translate::expr::{
    bind_and_rewrite_expr, expr_vector_size, translate_expr, walk_expr, BindingBehavior,
    WalkControl,
};
...
use turso_parser::ast::{self, CompoundOperator, CompoundSelect, Expr, ResultColumn};
...
fn translate_recursive_cte(
            ...
            ast::OneSelect::Select { columns, from, .. } => columns
                ...
                .flat_map(|(i, col)| match col {
                    ResultColumn::Expr(_, Some(alias)) => {
                        vec![alias.name().as_str().to_string()]
                    }
    ...
    if direct_cte_ref {
        ...
        let cte_outer_ref = OuterQueryReference {
            ...
            using_dedup_hidden_cols: crate::translate::plan::ColumnMask::default(),
    ...
    match base_case {
        ast::OneSelect::Select {
            ...
            if from.is_none() && where_clause.is_none() {
                ...
                let t_ctx = TranslateCtx::new(program, resolver.fork(), 0, false);
...
fn resolve_star_columns(
    ...
    if let ast::SelectTable::Table(qn, alias, _) = from_clause.select.as_ref() {
        let alias_str = alias.as_ref().map(|a| a.name().as_str());
