...
//! the index, the right defeats it with `+col`.
//!
...
fn in_transaction_insert_is_visible_through_the_index(tmp_db: TempDatabase) -> anyhow::Result<()> {
    ...
    assert_index_matches_scan(
        ...
        "count",
        "SELECT count(*) FROM v",
        "SELECT count(*) FROM (SELECT +id AS z FROM v)",
    );
    assert_index_matches_scan(
        &conn,
    ...
    assert_eq!(
        limbo_exec_rows(&conn, "SELECT count(*) FROM (SELECT +id AS z FROM v)"),
...
fn two_rows_that_collide_on_the_key_value(tmp_db: TempDatabase) -> anyhow::Result<()> {
    ...
    assert_eq!(
        limbo_exec_rows(&conn, "SELECT count(*) FROM v WHERE +k = 'a'"),
...
fn null_key_values_added_and_removed_in_the_transaction(
    ...
    assert_index_matches_scan(
        ...
        "SELECT count(*) FROM v",
        "SELECT count(*) FROM (SELECT +k AS z FROM v)",
    );
    conn.execute("ROLLBACK")?;
    Ok(())
}
...
fn rows_moving_across_a_filtered_views_predicate(tmp_db: TempDatabase) -> anyhow::Result<()> {
    ...
    assert_eq!(
        limbo_exec_rows(&conn, "SELECT id FROM v ORDER BY +k"),
