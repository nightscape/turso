...
impl<'a> LogicalPlanBuilder<'a> {
    ...
    fn build_select_table(&mut self, table: &ast::SelectTable) -> Result<LogicalPlan> {
        match table {
            ast::SelectTable::Table(name, alias, _indexed) => {
                let lookup_name = Self::normalized_name(&name.name);
    ...
    fn extract_equijoin_conditions(
        ...
        match expr {
            ast::Expr::Binary(lhs, ast::Operator::Equals, rhs) => {
                ...
                let left_expr = self.build_expr(lhs, &combined_schema)?;
                let right_expr = self.build_expr(rhs, &combined_schema)?;
                        ...
                        let col_belongs_to_table =
                            col.table.as_deref().map_or(false, |t| t == table_name)
                                || col
                                    .table_alias
                                    .as_deref()
                                    .map_or(false, |a| a == table_name);
                        if col_belongs_to_table {
    ...
    fn preprocess_aggregate_expressions(
        ...
        for agg_expr in aggr_exprs {
            ...
            } = agg_expr
            {
                ...
                for arg in args {
                    // Check if the argument is a simple column reference, a literal, or a complex expression
                    match arg {
                        ...
                        LogicalExpr::Literal(_) => {
                            // Literals are constants — no per-row evaluation needed.
                            // Pass through verbatim so downstream consumers (e.g. the
                            // IVM compiler's group_concat separator) can recognize them.
                            modified_args.push(arg.clone());
                        }
                        _ => {
                ...
                // Auto-lift simple Column references from the FILTER predicate
                // into the pre-projection. Without this, a filter that
                // references a column not in any aggregate arg would fail to
                // resolve when a sibling complex aggregate triggers a
                // pre-projection. The `filter` itself is passed through
                // unchanged — `compile_filter_predicate` resolves column refs
                // against the post-pre-projection schema.
                if let Some(f) = filter {
                    let mut filter_cols = Vec::new();
                    Self::collect_filter_columns(f, &mut filter_cols);
                    for col_expr in filter_cols {
                        if !pre_projection_exprs.iter().any(|e| e == &col_expr) {
                            let col_type = Self::infer_expr_type(&col_expr, input_schema)?;
                            if let LogicalExpr::Column(col) = &col_expr {
                                pre_projection_schema.push(ColumnInfo {
                                    name: col.name.clone(),
                                    ty: col_type,
                                    database: None,
                            ...
                            pre_projection_exprs.push(col_expr);
                        }
                    }
                }
    ...
    fn build_expr(&mut self, expr: &ast::Expr, _schema: &SchemaRef) -> Result<LogicalExpr> {
        match expr {
            ...
            ast::Expr::FunctionCallStar { name, filter_over } => {
                ...
                //
                // Reject window functions (OVER clause). Pre-existing bug: this arm
                // previously destructured `name, ..` and silently dropped both
                // filter_clause AND over_clause, so `count(*) OVER (...)` was
                // accepted by IVM. Now matches FunctionCall arm behavior.
                if filter_over.over_clause.is_some() {
                    return Err(LimboError::ParseError(
                        "Unsupported expression type: window functions are not yet supported"
                            .to_string(),
    ...
    fn parse_aggregate_function(name: &str, arg_count: usize) -> Option<AggregateFunction> {
        ...
        match_ignore_ascii_case!(match name_bytes {
            ...
            #[cfg(feature = "json")]
            b"JSON_GROUP_ARRAY" => Some(AggFunc::JsonGroupArray),
            #[cfg(feature = "json")]
            b"JSONB_GROUP_ARRAY" => Some(AggFunc::JsonbGroupArray),
            _ => None,
        })
    }
    ...
    // contains. Used by `preprocess_aggregate_expressions` to auto-lift filter
    // columns into the aggregate's pre-projection.
    fn collect_filter_columns(expr: &LogicalExpr, out: &mut Vec<LogicalExpr>) {
        match expr {
            LogicalExpr::Column(_) => out.push(expr.clone()),
            LogicalExpr::Literal(_) => {}
            LogicalExpr::BinaryExpr { left, right, .. } => {
                Self::collect_filter_columns(left, out);
                Self::collect_filter_columns(right, out);
            }
            LogicalExpr::UnaryExpr { expr, .. } => Self::collect_filter_columns(expr, out),
            LogicalExpr::IsNull { expr, .. } => Self::collect_filter_columns(expr, out),
            // Other expression shapes (function calls, CASE, IN, subquery, ...)
            // are unsupported in v1 FILTER predicates and rejected at compile
            // time by `compile_filter_predicate`. No need to walk into them
            // here; the rejection produces a clear error.
            _ => {}
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_table_star_in_join_filters_columns() {
        let schema = create_test_schema();
        let sql = "SELECT u.* FROM users u JOIN orders o ON u.id = o.user_id WHERE o.amount > 100";
        let plan = parse_and_build(sql, &schema).unwrap();

        match plan {
            LogicalPlan::Projection(proj) => {
                // users has 4 columns (id, name, age, email) — NOT 8 (all joined)
                assert_eq!(
                    proj.exprs.len(),
                    4,
                    "TableStar should only include columns from the starred table"
                );
                assert_eq!(proj.schema.columns.len(), 4);
                for col_info in &proj.schema.columns {
                    assert_eq!(
                        col_info.table.as_deref(),
                        Some("users"),
                        "All projected columns should belong to 'users', got {:?}",
                        col_info.table
                    );
                }
            }
