...
//! `core/translate/select.rs`, which runs each arm's plan through `optimize_plan` so an
//! equality predicate with a usable index is not left as a full-table `Operation::Scan`.
//! These tests assert the *plan shape* rather than timings, because timing assertions
//! are flaky under CI contention.
